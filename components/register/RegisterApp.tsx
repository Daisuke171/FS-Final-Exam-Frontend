...
export default function RegisterApp() {
    const { formData, handleChange, handleSubmit, error, success, loading } = useRegisterForm(); 
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4 w-full"> 
                    ...
                    <div className="flex flex-col md:flex-row md:gap-5"> 
                        ...
                        <div className="flex flex-col gap-3 w-full md:w-1/2">
                            <InputGroup icon={Mail} type="email" name="email" value={formData.email} onChange={handleChange} placeholder="Correo electrónico" />
                            <InputGroup icon={User} type="text" name="username" value={formData.username} onChange={handleChange} placeholder="Usuario" />
                            <InputGroup icon={Lock} type="password" name="password" value={formData.password} onChange={handleChange} placeholder="Contraseña" />
                        </div>

                        {/* COLUMNA 2 */}
                        <div className="flex flex-col gap-3 w-full md:w-1/2 mt-3 md:mt-0"> 
                            <InputGroup icon={UserPlus} type="text" name="name" value={formData.name} onChange={handleChange} placeholder="Nombre" />
                             <InputGroup icon={UserPlus} type="text" name="lastname" value={formData.lastname} onChange={handleChange} placeholder="Apellido" />
                             <InputGroup icon={Cake} type="date" name="birthday" value={formData.birthday} onChange={handleChange} placeholder="Fecha de Nacimiento" />
                        </div>
                    ...
                    >
                        {loading ? (
                            <span className="flex items-center justify-center text-sm">
                                <LoaderCircle size={18} className="animate-spin mr-2" /> REGISTRANDO
                            </span>
                        ) : (
                            "REGISTRARSE"
                        )}
                    ...
                    >
                        <img src="https://upload.wikimedia.org/wikipedia/commons/4/4a/Logo_Google_g_color.svg" alt="Logo de Google" className="w-4 h-4 mr-2" />
