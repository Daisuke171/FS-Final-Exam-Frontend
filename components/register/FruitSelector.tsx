...
export default function FruitSelector({ onVerificationChange }: FruitSelectorProps) {
    
   
    const CLASE_FONDO_ITEM = "bg-white/10 backdrop-blur-sm";

    const FRUIT_OPTIONS: FruitOption[] = useMemo(() => [
        { id: '1', type: 'Grape', icon: Grape, color: "text-sky-300", bg: CLASE_FONDO_ITEM, name: "Uva Morada" },
        { id: '2', type: 'Lemon', icon: Circle, color: "text-yellow-300", bg: CLASE_FONDO_ITEM, name: "Limón Amarillo" }, 
        { id: '3', type: 'Apple', icon: Apple, color: "text-red-300", bg: CLASE_FONDO_ITEM, name: "Manzana Roja" },
        { id: '4', type: 'Grape', icon: Grape, color: "text-indigo-300", bg: CLASE_FONDO_ITEM, name: "Uva Verde" }, 
    ], [CLASE_FONDO_ITEM]);
    ...
    const FruitItem: React.FC<{ fruit: FruitOption }> = React.memo(({ fruit }) => {
        ...
        return (
            ...
            >
                <fruit.icon size={28} className={fruit.color} /> 
            </div>
        );
    });
    ...
    return (
        ...
        <div className="w-full my-4 flex flex-col items-center p-3 rounded-2xl 
                    bg-blue-900/30 border border-sky-100/50 
                    shadow-inner shadow-sky-500/10 
            ...
            <p className="text-gray-400 text-xs mb-3 text-center font-semibold">
                {isVerified ? (
                    <span className="text-green-400 flex items-center justify-center hover-blue-glow">
                        <CheckCircle size={16} className="mr-1" /> ¡Verificación Humana Exitosa!
                    </span>
                ) : (
