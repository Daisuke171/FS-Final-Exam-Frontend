...
export default function CreateRoomModal({
  ...
  useEffect(() => {
    ...
    socket.on("roomCreated", (data: { roomInfo?: unknown; roomId: string }) => {
      console.log("Sala creada (handler):", data.roomInfo, "roomId:", data.roomId);
      setLoading(false);
      ...
      setCloseModal();
      router.push(`/games/turing-detective/${data.roomId}`);
    });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      console.log("Emit createRoom ->", payload);
      socketRef.current?.emit("createRoom", payload);

      // safety: if server doesn't respond with 'roomCreated', stop loading after timeout
      const fallback = window.setTimeout(() => {
        if (loading) {
          console.warn("createRoom: no response from server after 7s, clearing loading state");
          setLoading(false);
          setErrors({ general: "No response from server. Try again." });
        }
      }, 7000);
      // clear fallback when modal unmounts or on success
      const clearFallback = () => window.clearTimeout(fallback);
      // attach a one-time listener to clear
      socketRef.current?.once("roomCreated", () => clearFallback());
    } else {
