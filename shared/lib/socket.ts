...
export function getSocket(namespace: Namespace = "/") {
  ...
  if (!socketInstances[namespace]) {
    ...
    socketInstances[namespace].on("connect", () => {
      console.log(
        `✅ WS conectado [${namespace}]:`,
        socketInstances[namespace]?.id
      );
    });
    ...
    socketInstances[namespace].on("connect_error", (err) => {
      ...
      if (message && message.includes("Invalid namespace")) {
        console.warn(
          `⚠️ WS server does not expose namespace '${namespace}'. Cleaning up client socket to avoid repeated errors.`
        );
        try {
      ...
      const data =
        err && typeof err === "object" ? (err as any).data ?? null : null;
