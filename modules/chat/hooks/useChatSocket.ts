...
import {
  joinChat, leaveChat,
  onChatNew, offChatNew,
  onChatRead, offChatRead,
  onChatHistory, offChatHistory,
...
export function useChatSocket(chatId?: string, opts: Opts = {}) {
  ...
  useEffect(() => { rHist.current = opts.onHistory; }, [opts.onHistory]);
  useEffect(() => { rNew.current  = opts.onNew; }, [opts.onNew]);
  useEffect(() => { rRead.current = opts.onRead; }, [opts.onRead]);
